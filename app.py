        ...
        if st.button("🚀 Run AI Analysis"):
            ...
            with st.status("Running AI Analysis...", expanded=True) as status:
                ...
                **Instructions:**
                ...
                if not summary_code:
                    status.update(label="Failed to generate campaign summary code.", state="error", expanded=True)
                    st.stop()
                ...
                **Instructions:**
                ...
                if not adgroup_code:
                    status.update(label="Failed to generate ad group summary code.", state="error", expanded=True)
                    st.stop()
                ...
                if not date_col:
                    ...
                    **Instructions:**
                    ...
                    if not time_series_code:
                        status.update(label="Failed to generate campaign time-series code.", state="error",
                                      expanded=True)
                        st.stop()
                    ...
                    **Instructions:**
                    ...
                    if not adgroup_time_series_code:
                        status.update(label="Failed to generate ad group time-series code.", state="error",
                                      expanded=True)
                        st.stop()
...
if st.session_state.result_df is not None and not st.session_state.result_df.empty:
    ...
    if st.session_state.adgroup_df is not None and not st.session_state.adgroup_df.empty:
        ...
        st.dataframe(
            st.session_state.adgroup_df.head(10).style.format({
                'Spends': '{:,.2f}', 'Revenue': '{:,.2f}', 'ROAS': '{:.2f}x',
                'CPC': '{:,.2f}', 'CTR': '{:.2f}%'
            }),
            use_container_width=True,
    ...
    if st.session_state.selected_deepdive_tab == 0:
        if st.session_state.time_series_df is not None and not st.session_state.time_series_df.empty:
            ...
            selected_campaign = st.selectbox("Select a Campaign to Analyze:", campaign_list, key="campaign_select")
            ...
            if selected_campaign:
                ...
                if campaign_time_data.empty:
                    ...
                    st.subheader(f"Performance Trends for Campaign '{selected_campaign}'")
        ...
        if st.session_state.adgroup_time_series_df is not None and not st.session_state.adgroup_time_series_df.empty:
            ...
            selected_adgroup = st.selectbox("Select an Ad Group to Analyze:", adgroup_list,
                                            key="adgroup_select")

            if selected_adgroup:
                adgroup_time_data = st.session_state.adgroup_time_series_df[
                    st.session_state.adgroup_time_series_df['Ad Group'] == selected_adgroup
                    ].sort_values(by='Date')

                if adgroup_time_data.empty:
                    st.warning(f"No time-series data available for ad group: {selected_adgroup}")
                else:
                    # --- AI Insights ---
                    ...
                    col1, col2, col3 = st.columns(3)
                    with col1:
                        st.line_chart(adgroup_time_data, x='Date', y='Clicks', color="#FF4B4B")
                    with col2:
                        st.line_chart(adgroup_time_data, x='Date', y='CTR', color="#4BFF4B")
                    with col3:
                        st.line_chart(adgroup_time_data, x='Date', y='CPC', color="#4B4BFF")
